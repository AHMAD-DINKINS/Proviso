...
def implication(intVariables, boolVariables, post1, post2):
    set_option(max_args = 10000000, max_lines = 1000000, max_depth = 10000000, max_visited = 1000000)
    set_option(html_mode=False)
    set_fpa_pretty(flag=False)
    # print precondition
    logger = logging.getLogger("Runner.Framework.DisjunctLearner.z3Simplify")
    ...
    declareInts = "\n".join([ "(declare-const " + var + " Int)" for var in intVariables ])
    declareBools = "\n".join([ "(declare-const " + var + " Bool)" for var in boolVariables ])
    stringToParse = "\n".join([declareInts,  declareBools, "(assert " + post1 + ")"])

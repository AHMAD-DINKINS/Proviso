...
def sanitizePath(path, osType):
    try:
        ...
        if osType == "windows" or osType == "win":
            ...
            #runCommand( wslBin() + ' wslpath -a \'' + absPath + '\'')
            return absPath
...
def runCommand(args, directory = '.'):
    try:
        ...
        executionRun = subprocess.Popen(args, cwd = directory, stdout = subprocess.PIPE, stderr = subprocess.PIPE, shell=True)
...
def writeFile( location:str, fileContents:str):
    absPath = location.strip()
